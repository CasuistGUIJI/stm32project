...
uint1 MQTT_PacketSaveData(const int8 *devid, int16 send_len, int8 *type_bin_head, uint8 type, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	if(MQTT_PacketPublish(MQTT_PUBLISH_ID, "$sys/AL771S2CvP/temp1/thing/property/post", NULL, send_len, MQTT_QOS_LEVEL0, 0, 1, mqttPacket) == 0)  //Publish消息
	{
...
uint1 MQTT_PacketgetData(const int8 *devid, int16 send_len, int8 *type_bin_head, uint8 type, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	if(MQTT_PacketPublish(MQTT_PUBLISH_ID, "$sys/AL771S2CvP/temp1/thing/property/set", NULL, send_len, MQTT_QOS_LEVEL0, 0, 1, mqttPacket) == 0)  //Publish消息
	{
	}
	else
		return 1;
